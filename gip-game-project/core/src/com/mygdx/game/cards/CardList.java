...
public class CardList extends Array<Card> {
    ...
    public CardList(GipGameProject game, CardManager cardManager, TextureAtlas atlas) {
        this.game = game;
        this.cardManager = cardManager;
        float elapsed_time = 0;
        this.atlas = atlas;

        Card strike = new Card(0, "Strike", " Deal 6 Damage ", CardType.ATTACK, "green", 1, false, false, elapsed_time, game, atlas);
        add(strike);
        Card defend = new Card(1,"Defend", " Gain 5 Block ", CardType.SKILL, "green", 1, false, false,elapsed_time, game, atlas);
        add(defend);
        ...
        add(frontload);
         ...
         /*19: Card forloop = new Card(19, "For Loop", "The next card you\nENCODE causes X\nadditional copies\nto also be ENCODED.\nEXHAUST.", CardType.SKILL, "blue", 0, true, elapsed_time, game, atlas);
             addTo(forloop); */
        Card fragment = new Card(20, "Fragment", "Deal 4 Damage.\nGain 4 Block.\nENCODE.", CardType.ATTACK, "blue", 1,  false, false, elapsed_time, game, atlas);
        add(fragment);
        ...
        add(mutator);
        Card regret = new Card(22, "Regret", "UNPLAYABLE.\nAt the end of your\nturn, lose HP equal to\nthe number of cards in\nyour hand.", CardType.CURSE, "", 0,  false, false, elapsed_time, game, atlas);
        add(regret);
        Card clumsy = new Card(23, "Clumsy", "UNPLAYABLE.\nETHEREAL.", CardType.CURSE, "", 0,  false, false, elapsed_time, game, atlas);
        add(clumsy);
        Card reboot = new Card(24, "Reboot", "Gain 2 Energy.\nEXHAUST.", CardType.SKILL, "blue", 0, true, false, elapsed_time, game, atlas);
        add(reboot);
        Card iterate = new Card(25, "Iterate", "Deal 2 Damage 3\ntimes.\nENCODE.", CardType.ATTACK, "blue", 1,  false, false, elapsed_time, game, atlas);
        add(iterate);
        Card boost = new Card(26, "Boost", "Gain 6 BLOCK.\nENCODE.\nCOMPILE - \nGain 2 STRENGTH.", CardType.SKILL, "blue", 2,  false, false, elapsed_time, game, atlas);
        add(boost);
        Card autoshields = new Card(27, "Auto-Shields", "If you have no BLOCK,\ngain 11 BLOCK.", CardType.SKILL, "blue", 1,  false, false, elapsed_time, game, atlas);
        add(autoshields);
        Card doubleenergy = new Card(28, "Double Energy", "Double your energy.\nEXHAUST.", CardType.SKILL, "gold", 1, true, false, elapsed_time, game, atlas);
        add(doubleenergy);
    ...
    public Card getCard(int id) {
        for (Card card : this) {
            if (card.getId() == id) {
                return new Card(card.getId(), card.getTitle(), card.getDescription(), card.getCardType(), card.getRarity(), card.getCost(), card.isExhaust(), card.isUpgraded(), cardManager.getPlayerCards().size + 1, game,atlas);
            }
        }
        return null;
    }
